...
def most_common(frequencies: dict[str, int]) -> dict[str, int]:
    """Produces a dictionary with the ranking numbers with frequencies over 100."""
    result: dict[str, int] = {}
    for number in frequencies:
        if frequencies[number] >= 100: 
            result[number] = frequencies[number]
    return result